 ...
 */
function block_completion_progress_student_submissions($courseid, $userid = 0) {
    global $DB, $CFG;
...
function block_completion_progress_json($activities, $completions, $config, $userid, $courseid, $instance, $simple = false) {
    ...
    $progress = array();
    // Get colors and use defaults if they're not set in global
    $colornames = array(
                    'completed_color' => 'completed_color',
                    'submittednotcomplete_color' => 'submittednotcomplete_color',
                    'notCompleted_color' => 'notCompleted_color',
                    'futureNotCompleted_color' => 'futureNotCompleted_color',
    );
    $colors = array();
    ...
    for ($i = 0; $i < $numactivities; $i++) {
        if ($userid != $USER->id && array_key_exists($activities[$i]['type'], $alternatelinks) && 
            has_capability($alternatelinks[$activities[$i]['type']]['capability'], $activities[$i]['context'])) {
                $substitutions = array(
                                    '/:courseid/' => $courseid,
                                    '/:eventid/' => $activities[$i]['instance'],
                ...
                $link = $alternatelinks[$activities[$i]['type']]['url'];
                $link = preg_replace(array_keys($substitutions),
                        array_values($substitutions), $link);
                $activities[$i]['link'] = $CFG->wwwroot.$link;
            }
            else {
                $activities[$i]['link'] = $activities[$i]['url'];
